...
int main(int argc, char** argv)
{
    ...
    try {
        ...
        if (parser.has("image"))
        {
            ...
            cap.open(str);
            str.replace(str.end()-4, str.end(), ".jpg");
            outputFile = str;
        }
	...
	imshow("balabal", detectedFrame);

//* 	


     for ( size_t y=0; y<detectedFrame.rows; y++ )
      {
      
        for ( size_t x=0; x<detectedFrame.cols; x++ )
        {
            // 访问位于 x,y 处的像素
           double data=detectedFrame.at<Vec3b>(x,y)[2];
            //cout<<data<<endl;
	std::ofstream outFile; //打开文件 
	//outFile.open("./detected.txt"); 
	if (detectedFrame.at<Vec3b>(x,y)[2]==255&&detectedFrame.at<Vec3b>(x,y)[1]==0&&detectedFrame.at<Vec3b>(x,y)[0]==0)
             {  		
		cout<< x<<"  "<<y<<endl;		
		//outFile << x<<"  "<<y<<endl; 
             }  
...
void drawBox(Mat& frame, int classId, float conf, Rect box, Mat& objectMask)
{
   ...
   // putText(frame, label, Point(box.x, box.y), FONT_HERSHEY_SIMPLEX, 0.75, Scalar(0,0,0),1);
    ...
    drawContours(coloredRoi, contours, -1, (0,0,255), -5, LINE_8, hierarchy, 100);
